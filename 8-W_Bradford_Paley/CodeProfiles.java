...
public class CodeProfiles extends JFrame implements Runnable {
    ...
    Line[] readFile (String fileName) {  line(4);
        Vector<Line> v = new Vector<Line>();  line(10);
        try {  line(7);
            //  open the file   
    ...
    Vector<Point> pastPCPositions = new Vector<>(); int i13 = line(109);
    Vector<Point> pastFixationPositions = new Vector<>(); int i18 = line(177);
    Vector<Point> pastInsertionPositions = new Vector<>(); int i19 = line(184);
    ...
    public synchronized void drawAll (Graphics g) {  line(53);
        //  don't even try if we don't have a full-sized window yet /v2/img/enterproject.gif
        if (getSize().width < 21 || getSize().height < 21) {  line(255);
            return;
        }
        //--------------------------------------------------------------------------   
        ...
        getSize().width != offscreenImage.getWidth(this) ||

        getSize().height != offscreenImage.getHeight(this)) {  line(56);



            setFont(new Font("Courier", Font.PLAIN,

                Math.max(1, getSize().width / 320)));  line(78);



            offscreenImage = createImage(getSize().width, getSize().height);  line(57);
        ...
        if (cacheImage == null ||
            ...
            getSize().width != cacheImage.getWidth(this) ||

            getSize().height != cacheImage.getHeight(this)) {  line(59);

            cacheImage = createImage(getSize().width, getSize().height);  line(60);
        ...
        if (programLines != null) {  line(113);
            ...
            offscreenGraphics.drawString(
                ...
                programLines[drawnFixationLine].s.substring(0, drawnFixationPoint),
                    ...
                    (int) Math.min(getSize().width - lineDrawnLargerRatio *
    ...
    void drawCache (Graphics g) {
        ...
        g.fillRect(0, 0, getSize().width, getSize().height);  line(45);
    ...
    Cubic[][] calculateNaturalCubic (Vector<Point> v) {  line(153);
